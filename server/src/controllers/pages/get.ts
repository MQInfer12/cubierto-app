...
app.get("/pedir", async (req, res) => {
  ...
  const randomDonacion =
    donaciones[Math.floor(Math.random() * donaciones.length)];
...
app.get("/pedir/user", async (req, res) => {
  ...
  const randomDonacion =
    donaciones[Math.floor(Math.random() * donaciones.length)];
  const ofertas = filterOfertas(
    await xprisma.productoActivo.findMany({
      where: {
        producto: {
          usuario: {
            rol: "restaurante",
...
app.get("/restaurante/:idRestaurante", async (req, res) => {
  ...
  const ofertasActivas = filterOfertas(
    await xprisma.productoActivo.findMany({
      where: {
        producto: {
